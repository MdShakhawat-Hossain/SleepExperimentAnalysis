% function [figHandle,ax1,ax2,ax3,ax4,ax5,ax7] = Generate_Pupil_Sleep_Figures(procDataFileID,saveFigs)
...
filteredpupilMovement = filtfilt(sos1,g1,ProcData.data.Pupil.filtpupilMovement);
binPupilMovement = ProcData.data.Pupil.binpupilMovement;
...
pupil_Yvals = 1.30*max(indecesMax)*ones(size(binPupilMovement));
pupilInds = binPupilMovement.*pupil_Yvals;
...
yyaxis right
p2 = plot((1:length(filteredpupilMovement))/ProcData.notes.dsFs,filteredpupilMovement,'color','k','LineWidth',1);
ylabel('Pixels')

xlim([0,ProcData.notes.trialDuration_sec])
legend([p1,p2],'Force Sensor','PupilMovement')
...
s6 = scatter((1:length(binPupilMovement))/ProcData.notes.dsFs,pupilInds,'.','MarkerEdgeColor','r');
...
if (isempty(LPad_Yvals) == 1) && (isempty(RPad_Yvals) == 1) && (isempty(Aud_Yvals) == 1)
    ...
    legend([p5,s1,s2,s3,s4,s5,s6],'Pupil Diameter','movement','whisking','LSol','RSol','AudSol','pupilmovement')
end
