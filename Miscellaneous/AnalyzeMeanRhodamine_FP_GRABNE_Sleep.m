...
if firstHrs == "true"
    ...
    % keep only the data that occurs within the manually-approved awake regions
    [ACh_finalStimData_OLD,ACh_finalStimFileIDs_OLD,ACh_finalStimDuration_OLD,ACh_finalStimEventTimes_OLD] = RemoveInvalidData_IOS(ACh_stimData,ACh_stimFileIDs,ACh_stimDurations,stimEventTimes,ManualDecisions);
    [NE_finalStimData_OLD,NE_finalStimFileIDs_OLD,NE_finalStimDuration_OLD,NE_finalStimEventTimes_OLD] = RemoveInvalidData_IOS(NE_stimData,NE_stimFileIDs,NE_stimDurations,stimEventTimes,ManualDecisions);
    % keep only the data that doesn't have sleep
    SleepFileIDs = [];
    SleepBinTimes = [];
