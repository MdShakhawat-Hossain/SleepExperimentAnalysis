...
% if firstHrs == "true"
    % pull data from EventData.mat structure
    ACh_stimFilter = FilterEvents_IOS(EventData.GFP.P_ACh.stim,StimCriteriaA);
    NE_stimFilter = FilterEvents_IOS(EventData.GFP.P_NE.stim,StimCriteriaB);
    ...
    for nn = 1:size(ACh_ProcStimData,1)
        ACh_stimGFPMean{nn,1} = mean(ACh_ProcStimData(nn,(params.Offset + 1)*samplingRate:params.minTime.Stim*samplingRate),2);
        ACh_stimGFP{nn,1} = ACh_ProcStimData(nn,(params.Offset + 1)*samplingRate:params.minTime.Stim*samplingRate);
    end
    % take mean [GFP] 1-2 seconds after stimulation (right hem)
    for nn = 1:size(NE_ProcStimData,1)
        NE_stimGFPMean{nn,1} = mean(NE_ProcStimData(nn,(params.Offset + 1)*samplingRate:params.minTime.Stim*samplingRate),2);
        NE_stimGFP{nn,1} = NE_ProcStimData(nn,(params.Offset + 1)*samplingRate:params.minTime.Stim*samplingRate);
    end
    % save results
