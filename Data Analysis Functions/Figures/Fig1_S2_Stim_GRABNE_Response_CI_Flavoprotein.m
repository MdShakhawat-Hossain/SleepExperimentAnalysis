function [AnalysisResults] = Fig1_S2_Stim_GRABNE_Response_CI_Flavoprotein(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,FPanimalIDs)
...
title('Stimulus Evoked Response Contra')
%ACh
...
% plot(data.Ipsi.mean_timeVector,data.Ipsi.P_AChGFP_Mean + data.Ipsi.P_NEGFP_yCI95,'-','color',[0 0.4470 0.7410],'LineWidth',0.10)
 ylabel('\Delta F/F (%)')
 % legend('Flavoprotein')
legend([p2,p3],'CBV', 'Flavoprotein')

ax1.YAxis(1).Color = 'k';
% ax2.YAxis(2).Color = 'k';
xlabel('Peri-stimulus time (s)')
% legend([p1,p2,p3,p4],'CBV LH', 'CBV RH', 'NE', 'ACh')
axis square
set(gca,'box','off')
...
title('Stimulus Evoked Response Ipsi')
%ACh
...
if strcmp(saveFigs,'y') == true
    ...
    savefig(summaryFigureN,[dirpath ManipulationType '_' 'Stim-FiberSignals_response_CI']); %animalID   
    set(summaryFigureN,'PaperPositionMode','auto');
    print('-painters','-dpdf','-fillpage',[dirpath ManipulationType '_'  'Stim-FiberSignals_response_CI']) % animalID _NoSleep NoSleep_
    close 
end
