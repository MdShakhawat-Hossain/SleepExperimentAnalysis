function [AnalysisResults] = Fig4_FP_Transition_GRABNE_SingleMouse_CaMKIIa(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,FPanimalIDs)
...
for aa = 1:length(FPanimalIDs)
    ...
    if ~isfield(AnalysisResults.(animalID).Transitions,'REMtoAWAKE')
        transitions = {'AWAKEtoNREM','NREMtoAWAKE'};
    end
...
% Blood Volume and NE, ACh
    ...
    kn =  data.AWAKEtoNREM.AChCBV_Mean + data.AWAKEtoNREM.AChCBV_yCI95;

    hold on
    patch([T1 fliplr(T1)], [kn(1,:) fliplr(kn(2,:))], [0.8500 0.3250 0.0980],'FaceColor',[0.8500 0.3250 0.0980],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);    ylabel({'\DeltaF/F (%)'})
    % 
    p4 = plot(T1,data.AWAKEtoNREM.NECBV_Mean,'-','color',[0.6350 0.0780 0.1840],'LineWidth',2);
    hold on
    kn =  data.AWAKEtoNREM.NECBV_Mean + data.AWAKEtoNREM.NECBV_yCI95;
    patch([T1 fliplr(T1)], [kn(1,:) fliplr(kn(2,:))], [0.6350 0.0780 0.1840],'FaceColor',[0.6350 0.0780 0.1840],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);

    ylabel({'\DeltaF/F (%)'})
    
    % pC = plot(T1,data.AWAKEtoNREM.CBV_Mean,'-','color','red','LineWidth',2);
    % hold on
    % kn =  data.AWAKEtoNREM.CBV_Mean + data.AWAKEtoNREM.CBV_yCI95;
    % patch([T1 fliplr(T1)], [kn(1,:) fliplr(kn(2,:))], 'red','FaceColor','red','EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ...
    hold on
    NEn =  data.AWAKEtoNREM.NEGFP_Mean+ data.AWAKEtoNREM.NEGFP_yCI95;
    patch([T1 fliplr(T1)], [NEn(1,:) fliplr(NEn(2,:))],[0.4660 0.6740 0.1880],'FaceColor',[0.4660 0.6740 0.1880],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);

    p3 =  plot(T1,data.AWAKEtoNREM.AChGFP_Mean,'-','color',[0 0.4470 0.7410],'LineWidth',2);
    hold on
    NEn =  data.AWAKEtoNREM.AChGFP_Mean+ data.AWAKEtoNREM.AChGFP_yCI95;
    patch([T1 fliplr(T1)], [NEn(1,:) fliplr(NEn(2,:))],[0 0.4470 0.7410],'FaceColor',[0 0.4470 0.7410],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ylim([-5 5])
    yticks(-5:1:5)
    xline(0);
    ...
    legend([p1,p4,p2,p3],'LC CBV','Cort CBV','Cort','LC','Location','northeast','FontSize',5)
    xlim([-30,30])
    title('Awake to NREM')
    set(gca,'box','off')
    ax1.YAxis(1).Color = 'k';
...
%% NREM to Awake
    ...
    kn =  data.NREMtoAWAKE.AChCBV_Mean + data.NREMtoAWAKE.AChCBV_yCI95;
    ...
    hold on
    kn =  data.NREMtoAWAKE.NECBV_Mean + data.NREMtoAWAKE.NECBV_yCI95;
    patch([T1 fliplr(T1)], [kn(1,:) fliplr(kn(2,:))], [0.6350 0.0780 0.1840],'FaceColor',[0.6350 0.0780 0.1840],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ...
    % hold on
    % kn =  data.NREMtoAWAKE.CBV_Mean + data.NREMtoAWAKE.CBV_yCI95;
    % patch([T1 fliplr(T1)], [kn(1,:) fliplr(kn(2,:))], 'red','FaceColor','red','EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ...
    hold on
    NEn =  data.NREMtoAWAKE.NEGFP_Mean+ data.NREMtoAWAKE.NEGFP_yCI95;
    patch([T1 fliplr(T1)], [NEn(1,:) fliplr(NEn(2,:))],[0.4660 0.6740 0.1880],'FaceColor',[0.4660 0.6740 0.1880],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);

    p3 =  plot(T1,data.NREMtoAWAKE.AChGFP_Mean,'-','color',[0 0.4470 0.7410],'LineWidth',2);
    hold on
    NEn =  data.NREMtoAWAKE.AChGFP_Mean+ data.NREMtoAWAKE.AChGFP_yCI95;
    patch([T1 fliplr(T1)], [NEn(1,:) fliplr(NEn(2,:))],[0 0.4470 0.7410],'FaceColor',[0 0.4470 0.7410],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ylim([-5 5])
    yticks(-5:1:5)
    xline(0);
    ...
    % legend([p2,p3,pC],'NE','ACh','CBV','Location','northeast','FontSize',5)
...
if isfield(data,'NREMtoREM')
    ...
    kn =  data.NREMtoREM.AChCBV_Mean + data.NREMtoREM.AChCBV_yCI95;
    ...
    hold on
    kn =  data.NREMtoREM.NECBV_Mean + data.NREMtoREM.NECBV_yCI95;
    patch([T1 fliplr(T1)], [kn(1,:) fliplr(kn(2,:))], [0.6350 0.0780 0.1840],'FaceColor',[0.6350 0.0780 0.1840],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ...
    % hold on
    % kn =  data.NREMtoREM.CBV_Mean + data.NREMtoREM.CBV_yCI95;
    % patch([T1 fliplr(T1)], [kn(1,:) fliplr(kn(2,:))], 'red','FaceColor','red','EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ...
    hold on
    NEn =  data.NREMtoREM.NEGFP_Mean+ data.NREMtoREM.NEGFP_yCI95;
    patch([T1 fliplr(T1)], [NEn(1,:) fliplr(NEn(2,:))],[0.4660 0.6740 0.1880],'FaceColor',[0.4660 0.6740 0.1880],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);

    p3 =  plot(T1,data.NREMtoREM.AChGFP_Mean,'-','color',[0 0.4470 0.7410],'LineWidth',2);
    hold on
    NEn =  data.NREMtoREM.AChGFP_Mean+ data.NREMtoREM.AChGFP_yCI95;
    patch([T1 fliplr(T1)], [NEn(1,:) fliplr(NEn(2,:))],[0 0.4470 0.7410],'FaceColor',[0 0.4470 0.7410],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ylim([-5 5])
    yticks(-5:1:5)
    xline(0);
    ...
    ylabel({'\DeltaF/F (%)'})
    xlabel({'Time (s)'})
    % pq = plot(T1,data.NREMtoREM.AChGFP_Mean - data.NREMtoREM.NEGFP_Mean,'-','color',[0 0.2 0.1],'LineWidth',2);
...
if isfield(data,'REMtoAWAKE')
    ...
    kn =  data.REMtoAWAKE.AChCBV_Mean + data.REMtoAWAKE.AChCBV_yCI95;
    ...
    hold on
    kn =  data.REMtoAWAKE.NECBV_Mean + data.REMtoAWAKE.NECBV_yCI95;
    patch([T1 fliplr(T1)], [kn(1,:) fliplr(kn(2,:))], [0.6350 0.0780 0.1840],'FaceColor',[0.6350 0.0780 0.1840],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ...
    % hold on
    % kn =  data.REMtoAWAKE.CBV_Mean + data.REMtoAWAKE.CBV_yCI95;
    % patch([T1 fliplr(T1)], [kn(1,:) fliplr(kn(2,:))], 'red','FaceColor','red','EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ...
    hold on
    NEn =  data.REMtoAWAKE.NEGFP_Mean+ data.REMtoAWAKE.NEGFP_yCI95;
    patch([T1 fliplr(T1)], [NEn(1,:) fliplr(NEn(2,:))],[0.4660 0.6740 0.1880],'FaceColor',[0.4660 0.6740 0.1880],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);

    p3 =  plot(T1,data.REMtoAWAKE.AChGFP_Mean,'-','color',[0 0.4470 0.7410],'LineWidth',2);
    hold on
    NEn =  data.REMtoAWAKE.AChGFP_Mean+ data.REMtoAWAKE.AChGFP_yCI95;
    patch([T1 fliplr(T1)], [NEn(1,:) fliplr(NEn(2,:))],[0 0.4470 0.7410],'FaceColor',[0 0.4470 0.7410],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
    ylim([-5 5])
    yticks(-5:1:5)
    xline(0);
    ...
    ylabel({'\DeltaF/F (%)'})
    xlabel({'Time (s)'})
    % pq = plot(T1,data.REMtoAWAKE.AChGFP_Mean - data.REMtoAWAKE.NEGFP_Mean,'-','color',[0 0.2 0.1],'LineWidth',2);
...
if strcmp(saveFigs,'y') == true
    ...
    savefig(summaryFigure_D,[dirpath ManipulationType '_Transition_CBV_vs_GFP_Sleep_CI_CaMKIIa']);
    set(summaryFigure_D,'PaperPositionMode','auto');
    print('-painters','-dpdf','-fillpage',[dirpath ManipulationType '_Transition_CBV_vs_GFP_Sleep_CI_CaMKIIa'])
end
close
