function [figHandle,ax1,ax2,ax3,ax4,ax5,ax6] = GenerateSingleFigures_FP_GRABNE_BilateralECoG(procDataFileID,saveFigs)
...
EEG_RH = ProcData.data.cortical_RH.corticalSignal;
...
T_L = SpecData.cortical_LH.T;
F_L = SpecData.cortical_LH.F;

T_R = SpecData.cortical_RH.T;
F_R = SpecData.cortical_RH.F;
%% Yvals for behavior Indices
indecesMax = max([filtACh_CBV;filtNE_CBV]);%;filtNE_GFP;filtACh_GFP]);
...
title([animalID ' OptoStim Evoked Behavioral and Hemodynamic Changes: ' fileID2])
...
ylabel('\delta F/F')
legend([s1,s2,s3,s4,p5],'LSol','RSol','AudSol','OptoStim', 'CBV LH')
xlim([0,ProcData.notes.trialDuration_sec])
set(gca,'TickLength',[0,0])
set(gca,'Xticklabel',[])
set(gca,'box','off')
...
legend([s1,s2,s3,s4,p6],'LSol','RSol','AudSol','OptoStim', 'CBV RH')
...
%% Spectogram Left
ax7 = subplot(7,1,7);
Semilog_ImageSC(T_L,F_L,cortical_LHnormS,'y')
axis xy
c5 = colorbar;
ylabel(c5,'\DeltaP/P (%)')
clim([-100,100])
...
%% Spectogram Right
ax6 = subplot(7,1,6);
Semilog_ImageSC(T_R,F_R,cortical_RHnormS,'y')
axis xy
c5 = colorbar;
ylabel(c5,'\DeltaP/P (%)')
clim([-100,100])
