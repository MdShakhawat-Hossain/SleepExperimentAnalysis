function TDTFiberPhotometry_bilateralGRABNE_RawChange(filepath,rawDataFilespath)
...
% no z scoring for this data. We want to see the actual shift
            % [zScored_LH] =  ZScoreFiberData(lowPassData_LH,Params,'LH');
            % [zScored_RH] =  ZScoreFiberData(lowPassData_RH,Params,'RH'); 
            zScored_LH = lowPassData_LH;
            zScored_RH = lowPassData_RH;
            ...
            hold on;
            plot(figTime,filtfilt(Params.sos_plot,Params.g_plot,CBVCorrection_RH.F405))
            legend('RH 465','RH 405')
            title('405 vs 465'); xlabel('Time (min)'); xlim([0 figTime(end)]);
            ylabel('Raw Fluoresence');

            k(2) = subplot(4,1,2);
            plot(figTime,filtfilt(Params.sos_plot,Params.g_plot,CBVCorrection_LH.F465))
            hold on;
            plot(figTime,filtfilt(Params.sos_plot,Params.g_plot,CBVCorrection_LH.F405))
            legend('LH 465','LH 405')
            title('405 vs 465'); xlabel('Time (min)'); xlim([0 figTime(end)]);
            ylabel('Raw Fluoresence');
...
%             close 
            %% house keeping
            timeN = time_Trim; % remove first 210 seconds;
            sampleN = floor(timeN*Params.DataFs);
