...
filteredpupildiameter = hampel(filteredpupildiameter,resample_rate*5);

% filteredpupildiameter = medfilt1(filteredpupildiameter,resample_rate*2);
% filteredpupildiameter = filloutliers(filteredpupildiameter,"clip",'movmean',resample_rate*2);
% force sensor
...
EMGSignal = filtfilt(sos1,g1,resample(ProcData.data.EMG.emgSignal,resample_rate,ProcData.notes.dsFs));
% stimulations
...
% ACh_Rhodamine = resample(ProcData.data.Rhodamine.P_ACh,resample_rate,ProcData.notes.dsFs);
...
% yyaxis right
p2 = plot((1:length(filtForceSensor))/(resample_rate*60),filtForceSensor,'color','k','LineWidth',1);
ylabel('Force')
...
% % Whisker angle
ax2 = subplot(9,1,3:4);
p1 = plot((1:length(filteredWhiskerAngle))/(resample_rate*60),-filteredWhiskerAngle,'color',colors('dark pink'),'LineWidth',1);
hold on
ylabel('Angle (deg)')
% yyaxis right
...
ax3 = subplot(9,1,6:7);
s3 = scatter(LPadSol/60,LPad_Yvals,'v','MarkerEdgeColor','k','MarkerFaceColor','c'); hold on;
s4 = scatter(RPadSol/60,RPad_Yvals,'v','MarkerEdgeColor','k','MarkerFaceColor','m');
s5 = scatter(AudSol/60,Aud_Yvals,'v','MarkerEdgeColor','k','MarkerFaceColor','g');
p5 = plot((1:length(filteredpupildiameter))/(resample_rate*60),filteredpupildiameter,'color',colors('dark candy apple red'),'LineWidth',1);
ylabel('Diameter (Z)')
